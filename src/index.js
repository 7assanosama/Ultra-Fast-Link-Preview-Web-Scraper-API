...
async function handleSingle(target, ctx) {
  ...
  try {
    ...
    if (size > 5_000_000) return json({ error: "Page too large" }, 413);
...
// 🧠 ADVANCED METADATA EXTRACTION WITH FALLBACKS
function extractMeta(html, url) {
  const get = regex => html.match(regex)?.[1] || null;

  // Basic OG/Twitter/Standard
  ...
  // Author fallback
  let author = get(/name="author" content="(.*?)"/i) || get(/article:author" content="(.*?)"/i) || get(/twitter:creator" content="(.*?)"/i);
  if (!author) {
    const authorTag = html.match(/<a[^>]+rel=["']author["'][^>]*>(.*?)<\/a>/i);
    if (authorTag) author = authorTag[1].trim();
  }

  // Keywords fallback
  let keywords = get(/name="keywords" content="(.*?)"/i) || get(/article:tag" content="(.*?)"/i);
  if (!keywords) keywords = null;

  const theme_color = get(/name="theme-color" content="(.*?)"/i);
  let type = get(/og:type" content="(.*?)"/i);
  if (!type) type = null;

  // Locale fallback
  let locale = get(/og:locale" content="(.*?)"/i);
  if (!locale) {
    const htmlLang = html.match(/<html[^>]+lang=["'](.*?)["']/i);
    if (htmlLang) locale = htmlLang[1].trim();
  }

  // Video/Audio fallback
  let video = get(/og:video:url" content="(.*?)"/i) || get(/og:video" content="(.*?)"/i);
  if (!video) {
    const vidTag = html.match(/<video[^>]+src=["'](.*?)["']/i);
    if (vidTag) video = vidTag[1].trim();
  }

  let audio = get(/og:audio" content="(.*?)"/i);
  if (!audio) {
    const audTag = html.match(/<audio[^>]+src=["'](.*?)["']/i);
    if (audTag) audio = audTag[1].trim();
  }

  // Robots fallback
  let robots = get(/name="robots" content="(.*?)"/i);
